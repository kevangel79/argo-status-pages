...
import { ResultServicesT, ReportT } from "../types";
...
const Uptime = (props: any) => {
  ...
  useEffect(() => {
    getResultServicesRanged(90).then((response: ResultServicesT) =>
    ...
    if (results && results.length > 0) {
      ...
      for (var i = startidx; i < results.length; i++) {
        if (results[i].unknown === 1) {
          ...
          if (
            (parseFloat(thresholds.uptime) + 0.01 < 1 &&
              results[i].uptime <= thresholds.uptime + 0.01) ||
            (parseFloat(thresholds.availability) + 2 < 100 &&
              results[i].availability <= thresholds.availability + 2) ||
            (parseFloat(thresholds.reliability) + 2 < 100 &&
              results[i].reliability <= thresholds.reliability + 2)
          )
            fill = "#e67e22";
          else {
            fill = "#16a085";
        ...
        vec.push(
          <rect
            key={`day-${i}`}
            onMouseOver={mouseOver.bind(this, results[i])}
            onMouseOut={mouseOut}
            height="34"
            width="5"
            x={`${marginx}`}
            y="0"
            fill={fill}
            className={`uptime-day component-0mw0pdgvs1l3 day day-${i}`}
            data-html="true"
          ></rect>,
        );
        vec.push(
  ...
  const tooltip = (
    <div>
      <div
        className={`${styles["pointer-container"]}`}
        style={{ top: 100, left: coords.x - 8 }}
      >
      ...
      <div
        ...
        style={{ top: 110, left: coords.x - 160 }}
      >
          ...
          <div style={{ textAlign: "initial", marginTop: "0.5rem" }}>
            <ul>
              <li>
                <span>
                  Uptime:{" "}
                  {parseFloat((result.uptime * 100).toString()).toFixed(2)} %
                </span>
              </li>
              <li>
                <span>
                  Availability: {parseFloat(result.availability).toFixed(2)} %
                </span>
              </li>
              <li>
                <span>
              ...
              <li>
                <span>Downtime: {result.downtime}</span>
              </li>
            </ul>
          </div>
        </div>
      ...
      <div className="card-body">
        ...
        <div className={`${styles["uptime-90-days-wrapper"]}`}>
          {windowSize.innerWidth > 800 ? (
            <svg
              id="uptime-component-0mw0pdgvs1l3"
          ...
          <div className={`${styles["legend"]}`}>
            <div
              className={`${styles["legend-item"]} ${styles["light"]} ${styles["legend-item-date-range"]}`}
            >
              {windowSize.innerWidth > 800 ? (
                <div>
                  <span
                    className={`${styles["availability-time-line-legend-day-count"]}`}
                  >
            ...
            <div
              className={`${styles["legend-item"]} ${styles["legend-item-uptime-value"]}`}
            >
              <span id="uptime-percent-0mw0pdgvs1l3">
                {windowSize.innerWidth > 800 ? (
                  <var data-var="uptime-percent">
                    {calculateAverageUptime(results)}
                  </var>
                ) : (
                  <var data-var="uptime-percent">
                    {calculateAverageUptime(results && results.slice(-50))}
                  </var>
                )}
              </span>
              % uptime
